...
class flight_data(object):
    ...
    def _create_constraints(self):
        with self.driver.session() as session:
            ...
            session.run(
                "CREATE CONSTRAINT unique_airline IF NOT EXISTS FOR (a:Airline) REQUIRE a.airline_name IS UNIQUE")
            session.run(
                "CREATE CONSTRAINT unique_destiny IF NOT EXISTS FOR (d:Destiny) REQUIRE d.destiny_name IS UNIQUE")
    ...
    def _create_airline_node(self, airline_name):
        with self.driver.session() as session:
            try:
                session.run(
                    "CREATE (a:Airline {airline_name: $airline_name})", airline_name=airline_name)
            except ConstraintError:
                print(eval(ConstraintError))
                pass
    ...
    def _create_destiny_node(self, destiny_name):
        with self.driver.session() as session:
            try:
                session.run(
                    "CREATE (d:Destiny {destiny_name: $destiny_name})", destiny_name=destiny_name)
            except ConstraintError:
                print(eval(ConstraintError))
                pass
    ...
    def _create_reason_node(self, reason_name):
        with self.driver.session() as session:
            try:
                session.run(
                    "CREATE (r:Reason {reason_name: $reason_name})", reason_name=reason_name)
            except ConstraintError:
                print(eval(ConstraintError))
                pass
    ...
    def _create_stay_node(self, stay_name):
        with self.driver.session() as session:
            try:
                session.run(
                    "CREATE (s:Stay {stay_name: $stay_name})", stay_name=stay_name)
            except ConstraintError:
                print(eval(ConstraintError))
                pass
    ...
    def init(self, source):
        with open(source, newline='') as csv_file:
            ...
            for r in reader:
                ...
                self._create_movie_node(
                    r["movie_name"], r["movie_category"], r["movie_actor"])
                self._create_actor_node(r["movie_actor"])
                self._create_category_node(r["movie_category"])
                self._create_user_to_movie_relationship(
                    r["username"], r["movie_name"])
                self._create_actor_to_movie_relationship(
                    r["movie_actor"], r["movie_name"])
                self._create_movie_to_category_relationship(
                    r["movie_name"], r["movie_category"])
                self._create_user_to_category_relationship(
                    r["username"], r["movie_category"])
                self._create_category_to_actor_relationship(
                    r["movie_category"], r["movie_actor"])
                self._create_user_to_actor_relationship(
                    r["username"], r["movie_actor"])
